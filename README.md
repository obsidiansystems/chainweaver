...
In order to deploy the remote verifying server along with the chainweaver deployment copy the file `pact-server/module.nix` to the `deploydir`.
Based on the deployment target please edit the imports in this file to use either `mkBaceEc2` or `virtualbox-image.nix`.

The other optional configurable parameter is this file is the `location`, which is the nginx virtualhost's path.
Its default value is `/pact/`, but it could be modified to some other value.

Finally make sure the `config/common/verification-server` file matches the location value specified in `module.nix`.
